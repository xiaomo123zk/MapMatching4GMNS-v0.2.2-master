...
public:
  ...
  void BuildGridSystem()
  {
    ...
    if (_MAX_GRID_SIZE < g_node_vector.size() / 1000)
    {
      g_grid_size = _MAX_GRID_SIZE;
    }
    else if (_MAX_GRID_SIZE >= g_node_vector.size() / 1000)
    {
      g_grid_size = g_node_vector.size() / 1000;
      if (g_grid_size < 1)
      {
        g_grid_size = 1;
      }
...
void g_ReadInputData(int mode)
{
  ...
  if (mode == 0 && gps_parser.OpenCSVFile("input_agent.csv", true))
  {
    double x, y;
    string time_stamp;
    int time_in_second;
  ...
  else
  {
    ...
    gps_point_count = 0;
    if (mode == 1 && gps_parser.OpenCSVFile("trace.csv", true))
    {
      double x, y;
      string time_stamp;
      int time_in_second;
